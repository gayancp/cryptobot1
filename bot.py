...
def norm_and_preprocess():
    ...
    # TODO: Preprocess and normalize the data to be ready for inputting to the model
...
def calculate_technical_indicators():
    ...
    try:    
        for index, row in df.iterrows():
            ...
            # Calculate SMA
            sma_value = calculate_sma(df, interval, timestamp)
            sma_values.append(sma_value)

            # Calculate RSI
            rsi_value = calculate_rsi(price_movements, interval, timestamp, period)
            rsi_values.append(rsi_value)

            # Calculate MACD
            macd_value = calculate_macd(price_movements, interval, timestamp)
            macd_values.append(macd_value)

            # Calculate Bollinger Bands
            upper_band, middle_band, lower_band = calculate_bollinger_bands(price_movements, interval, timestamp, period)
            upper_bands.append(upper_band)
            middle_bands.append(middle_band)
            lower_bands.append(lower_band)

            # Calculate Average True Range
            atr_value = calculate_atr(price_movements, interval, timestamp, period)
            atr_values.append(atr_value)

            # Calculate Stochastic Oscillator
            slowk, slowd = calculate_stochastic_oscillator(price_movements, interval, timestamp, period)
            slowks.append(slowk)
            slowds.append(slowd)

            # Calculate Moving Average Convergence Divergence Histogram
            macd_hist_value = calculate_macd_histogram(price_movements, interval, timestamp)
            macd_hist_values.append(macd_hist_value)

            # Calculate On-Balance Volume
            obv_value = calculate_on_balance_volume(price_movements, interval, timestamp)
            obv_values.append(obv_value)

            # Calculate Ichimoku Cloud
            tenkan_sen, kijun_sen, senkou_span_a, senkou_span_b = calculate_ichimoku_cloud(price_movements, interval, timestamp)
            ...
            cmf_value = calculate_chaikin_money_flow(price_movements, interval, timestamp, period)
            cmf_values.append(cmf_value)

            # Calculate Williams %R
            willr_value = calculate_williams_percent_r(price_movements, interval, timestamp, period)
    ...
    df12hour['sma'] = sma_values
    df12hour["rsi"] = rsi_values
    df12hour["macd"] = macd_values
    df12hour["bollinger_bands"] = bollinger_bands
    df12hour["average_true_range"] = atr_values
    df12hour["stochastic_oscillator"] = stochastic_oscillator
    df12hour["Moving_Average_Convergence_Divergenc_Histogram"] = macd_hist_values
    df12hour["On_Balance_Volume"] = obv_values
    df12hour["Ichimoku_Cloud"] = Ichimoku_Cloud
    df12hour["Chaikin_Money_Flow"] = cmf_values
    df12hour["Williams_%R"] = willr_values
...
def define_models():
    ...
    combined_model.add(Merge([cnn_model, lstm_model], mode='concat'))
...
end_time = pd.to_datetime('2021-01-01 11:59:59')

# Step 5: Define the training loop
for i in range(96):
    while True:        
        #get 12 hour time
        df12hour = df.loc[(df['timestamp'] >= start_time) & (df['timestamp'] <= end_time)].copy()

        #technical indicator calculation
        calculate_technical_indicators()

        input_data = df12hour.values
        ...
        start_time += pd.Timedelta(seconds=30)
